/**
 * @file amu_regs.h
 * @brief AMU TWI/I2C register map and register access helpers
 *
 * @author CJM28241
 * @date 5/5/2019
 */
...
typedef enum amu_reg_system_t  {
	...
	AMU_REG_SYSTEM_STATUS_HRADC = AMU_REG_SYSTEM_OFFSET + 0x08,			/*!< uint32_t - HRADC ID in lower 16 bits, ADC status in upper 16; layout depends on the ADC (AD7124 on legacy, ADS131M0x on AMU3) */
} AMU_REG_SYSTEM_t;
...
typedef enum amu_reg_adc_data_t {
	...
	AMU_REG_ADC_DATA_BIAS = AMU_REG_ADC_DATA_REG_OFFSET + 0x14,		/*!< @amuhw{Legacy only} float - converted value of bias voltage measurement, generated by uController DAC */
	AMU_REG_ADC_DATA_OFFSET = AMU_REG_ADC_DATA_REG_OFFSET + 0x18,		/*!< @amuhw{Legacy only} float - converted value of offset voltage measurement, generated by uController DAC */
	AMU_REG_ADC_DATA_TEMP = AMU_REG_ADC_DATA_REG_OFFSET + 0x1C,		/*!< @amuhw{Legacy only} float - converted value of internal ADC temperature sensor */
	AMU_REG_ADC_DATA_AVDD = AMU_REG_ADC_DATA_REG_OFFSET + 0x20,		/*!< @amuhw{Legacy only} float - converted value of internal ADC AVDD voltage */
	AMU_REG_ADC_DATA_IOVDD = AMU_REG_ADC_DATA_REG_OFFSET + 0x24,		/*!< @amuhw{Legacy only} float - converted value of internal ADC IOVDD voltage */
	AMU_REG_ADC_DATA_ALDO = AMU_REG_ADC_DATA_REG_OFFSET + 0x28,		/*!< @amuhw{Legacy only} float - converted value of internal ADC ALDO voltage */
	AMU_REG_ADC_DATA_DLDO = AMU_REG_ADC_DATA_REG_OFFSET + 0x2C,		/*!< @amuhw{Legacy only} float - converted value of internal ADC DLDO voltage */
...
typedef enum amu_reg_sweep_config_t {
	...
	AMU_REG_SWEEP_CONFIG_AM0 = AMU_REG_SWEEP_CONFIG_OFFSET + 0x08,					/*!< float - sun power (default 1366.1 W/m^2 based on ASTM E-490) */
	AMU_REG_SWEEP_CONFIG_AREA = AMU_REG_SWEEP_CONFIG_OFFSET + 0x0C,				/*!< float - cell area as defined by user (2x2 = 0.0004 m^2) */
...
typedef enum amu_reg_sweep_meta_t {
	AMU_REG_SWEEP_META_VOC = AMU_REG_SWEEP_META_OFFSET+ 0x00,					/*!< float - converted Voc value of sweep */
	AMU_REG_SWEEP_META_ISC = AMU_REG_SWEEP_META_OFFSET+ 0x04,					/*!< float - converted Isc value of sweep  */
	...
	AMU_REG_SWEEP_META_FF = AMU_REG_SWEEP_META_OFFSET+ 0x10,					/*!< float - calculated Fill Factor based on sweep data */
	AMU_REG_SWEEP_META_EFF = AMU_REG_SWEEP_META_OFFSET+ 0x14,					/*!< float - calculated Efficiency based on sweep data */
	AMU_REG_SWEEP_META_VMAX = AMU_REG_SWEEP_META_OFFSET+ 0x18,				/*!< float - calculated Vmax based on sweep data */
	AMU_REG_SWEEP_META_IMAX = AMU_REG_SWEEP_META_OFFSET+ 0x1C,				/*!< float - calculated Imax based on sweep data */
...
typedef enum amu_reg_data_ptr_t {
	...
	AMU_REG_DATA_PTR_PRESSURE = AMU_REG_DATA_PTR_OFFSET + 0x08, 		/*!< @amuhw{EYAS only} Max determined by TWI data definition, partially memory dependent*/
...
/** @brief Register capacity: static map size, full AMU_TRANSFER_REG_SIZE for AMU_REG_TRANSFER_PTR.
 *         Use amu_reg_get_length() for the length a read should serve */
